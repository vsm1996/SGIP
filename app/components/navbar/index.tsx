      ...
      <ul className='w-full flex items-center max-sm:justify-between'>
        ...
        {status === 'authenticated' && (
          <li className='flex justify-between items-center sm:w-full space-x-3'>
            <span className='text-lg font-extrabold max-sm:hidden'>Welcome, {session.user!.username || session.user!.firstName || session.user!.name}</span>
            <span className='flex items-center'>
              <Link tabIndex={-1} target="_blank" href='https://www.youtube.com/@george128306' className='link link-hover mr-5'>Video Lectures</Link>
