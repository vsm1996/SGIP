...
const ThemeController = () => {
  ...
  const handleThemeChange = (e: ChangeEvent<HTMLInputElement>) => {
    const selectedTheme = e.target.checked ? 'synthwave' : 'aqua';
  ...
  return (
    <div>
      <label className={`swap swap-rotate ${theme === 'synthwave' ? 'hover:animate-[pulse_2s_ease-in-out_infinite]' : 'hover:animate-[spin_2s_ease-in-out_infinite]'}`}>
        <input type="checkbox" className="theme-controller" checked={theme === 'synthwave'} onChange={handleThemeChange} id='themeControllerField' />
